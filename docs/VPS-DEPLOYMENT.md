...
The installer supports Debian 12 and 13. A reasonable pilot VPS is 8 vCPU, 16 GiB RAM, and 200–300 GiB NVMe. The scanner container is limited to 4 CPUs/8 GiB, so reduce its Compose limits if the host is smaller.
...
The generated database, Grafana, and API credentials are ready without editing. Never put secret values in `.env`, manifests, model packets, Discord, or Git.
...
The test command builds an isolated test image and uses only SQLite under a temporary container filesystem. It performs no target network calls. `preflight --build` downloads and builds images but starts no service or scan. The scanner image verifies release checksums for Nuclei/Gitleaks and fails its build if Gitleaks cannot detect a synthetic fake token.
...
## 5. Add a program, then start

Follow [Program onboarding](PROGRAM-ONBOARDING.md). With no approved `.yml` manifest, the stack is intentionally idle.
